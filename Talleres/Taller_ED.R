...
tis = seq(0,2,by = 0.1)
sol = ode(c(2,-1),tis,funp,parms = NULL, method = "Runge-kutta_4_Grado")
tabla = cbind(tis, sol[,2])
tabla
plot(tis, sol[,2], main="Runge-Kutta")
...
#Usando h=0.2
tis2 = seq(0,4,by = 0.2)
sol2 = ode(c(2,-1),tis2,funp,parms = NULL, method = "rk4")
...
tis = seq(0,1,by = 0.1)
sol = ode(c(3,6),tis,funp,parms = NULL, method = "euler")
...
#Veinte puntos de la solucion
funp= function(x,datos,parms){
...
sol = ode(c(1,2),xis,funp,parms = NULL, method = "rk4")
...
sol = ode(c(0,1),tis,funp,parms = NULL, method = "rk4")
tabla = cbind(tis, sol[,2],sol[,3])
print(tabla)
...
funpDespues= function(t,y,parms){
  dz = -4*y[2] 
  dy = y[1]
  return (list(c(dz,dy)))
}
...
sol2 = ode(c(sol[,2][21],sol[,3][21]),tis2,funpDespues,parms = NULL, method = "rk4")
...
curve(solReal, add = T, col='chartreuse4', from = 0, to=2*pi)

solRealDespues = function(t){
  rr = 0.983573*cos(2*t)+3.14884*sin(t)*cos(t)
}
curve(solRealDespues, add = T, col='chartreuse4', from =2*pi, to=4*pi)

#---------------------Punto 8-------------------------------
#x' = x -3y
#y' = 3x+7y
#Veinte puntos de la solución, gráfica e interpolación
funp= function(t,datos,parms){
...
sol = ode(c(1,1),tis,funp,parms = NULL, method = "rk4")
